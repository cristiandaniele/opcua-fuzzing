...
unsigned int* extract_response_codes_opcua(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    if (mem_count >= 8) {
      ...
      // Bytes 8-11: error number
      
      if ((mem[0] = 'E') && (mem[1] == 'R') && (mem[2] == 'R')) {
        // If message type is ERR, we use the second byte from error code (endianness swapped), as this value has the most variance.
        message_type = mem[10];
      } else { // If message is not an ERR, interpret it as status code 0x00
        message_type = 0x00;
      }
